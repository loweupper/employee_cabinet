...
from typing import List # Для типизации параметров запросов
...
from modules.admin.service import CategoryMappingService # Сервис для управления маппингом категорий
...
# ===================================
@router.get("/mappings/category-departments")
async def get_category_department_mappings(
    user: User = Depends(get_current_user_from_cookie),
    db: Session = Depends(get_db)
):
    """Получить все маппинги категорий на отделы"""
    if user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Access denied")
    return CategoryMappingService.get_all_mappings(db)


@router.put("/mappings/category-departments/{category}")
async def update_category_department_mapping(
    category: str,
    ...
    if user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Access denied")
    result = CategoryMappingService.update_mapping(category, departments, db)
    if not result:
