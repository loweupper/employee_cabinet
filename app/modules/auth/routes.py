...
async def login_page(request: Request, registered: bool = None, error: str = None):
    """
    ...
    return templates.TemplateResponse(
        "web/auth/login.html",
        {"request": request, "success": success_message, "error": error},
    )
...
@router.get(
    ...
    description="Страница для регистрации нового пользователя",
...
@router.post(
    ...
    description="Регистрация нового пользователя через веб-форму",
)
...
@router.post(
    ...
    description="Логин пользователя через веб-форму с email и паролем",
)
    ...
    try:
        ...
        try:
            ...
            await tracker.record_attempt(
                email, client_ip, True, user.id if user else None
            )
...
@router.post(
    ...
    responses={
        401: {
            "model": ErrorResponse,
            "description": "Невалиден или истёк refresh token",
        }
    },
)
...
@router.get(
    "/me", response_model=UserRead, summary="Получить профиль текущего пользователя"
)
...
@router.patch("/me", response_model=UserRead, summary="Обновить свой профиль")
...
@router.post(
    ...
    responses={400: {"model": ErrorResponse, "description": "Текущий пароль неверен"}},
)
def change_password(
...
def get_sessions(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    sessions = (
        ...
        .filter(SessionModel.user_id == user.id, SessionModel.is_revoked == False)
        .all()
    )
    return sessions
...
@router.delete(
    ...
    responses={404: {"model": ErrorResponse, "description": "Сессия не найдена"}},
)
def revoke_session(
    session_id: int,
    user: User = Depends(get_current_user),
